...
export const responseInterceptor = async (response: AxiosResponse) => {
    if (response.status === 406) {
        if (isRefreshing) {
          ...
          const handleRefreshCall = await handleRefreshToken(sessionAuth?.accessToken, sessionAuth?.refreshToken, currentUser?.roleId)
          if (handleRefreshCall.accessToken && currentUser) {
            ...
            store.dispatch(sessionActions.doLogout({
        ...
        if(!isLogout) {
          const sessionAuth = JSON.parse(await localStorage.getItem(STORAGE_KEY) || "").session?.data?.auth;
