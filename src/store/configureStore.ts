...
export function configureAppStore() {
    ...
    store.subscribe(throttle(() => {
        ...
        if (!persistedStateCache?.session?.data?.auth?.rememberMe) {
            if (store.getState().session?.data?.auth && Object.keys(store.getState().session?.data?.auth || {}).length > 0) {
        ...
        saveState({
                        ...
                        store.getState().session ?
                            {
                                ...
                                auth: {
                                    ...(store.getState().session.data.auth?.rememberMe ? store.getState().session.data.auth : {})
                                }
                            } :
            ...
            config: {
                data: {
                    ...(store.getState().config ? store.getState().config.data : persistedStateCache?.config?.data)
                }
            },
            device: {
