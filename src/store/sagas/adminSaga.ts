...
import {
  apiDoUploadImageMenuFile,
  apiDoUploadExcelMenuFile,
  apiAdminMenuImport,
  apiDoFetchOutletList,
  apiDoFetchMenusOutlet,
  apiGetAllThirdParty,
  apiGetExportDataMenu,
  apiDoActiveOutletPartner,
  apiDoFetchMenuPartnerData,
  apiDoDeactivateOutletPartner,
  apiDoFetchOutlet,
...
export function* doUploadMenu({ payload }: any) {
  try {
    ...
    const excelResponse = yield call(apiDoUploadExcelMenuFile, {
      file: payload.menuFile,
    ...
    const productImages = excelData
      .filter((row) => row.productImages && row.productImages !== '')
      .reduce((prev, curr) => {
        const productImagesCell = curr.productImages.split(',');
        return [...prev, ...productImagesCell];
      }, []);
    ...
    if (uploadImageList.length >= (payload.imageFiles || []).length) {
      const missingImages = uploadImageList.filter((item, index) => {
        return (
          payload.imageFiles.findIndex((file) => {
    ...
    const allowedImages = payload.imageFiles.filter(
      (item) => uploadImageList.indexOf(item.name) >= 0
    );
    ...
    const [...imageResponses] = yield all([...imageTask]);
...
export function* doFetchOutletList({ payload }: any) {
  try {
    ...
    if (response.data && response.data.status) {
      ...
      yield put(actions.loadedOutletList(adminOutletPayload));
    } else {
...
export function* doActiveOutletPartner({
  payload,
}: {
  type: string;
  ...
  try {
    ...
    if (response.data && response.data.status) {
      yield put(
        actions.doFetchMenuPartnerData({
          token: payload.token,
...
export function* doDeactivateOutletPartner({
  payload,
}: {
  type: string;
...
export function* doFetchMenuPartnerData({
  payload,
}: {
  type: string;
  ...
  try {
    ...
    if (response.data && response.data.status) {
      ...
      yield put(
        actions.fetchMenuPartnerDataError({
          outletId: payload.outletId,
          menuId: payload.menuId,
        })
      );
...
export function* doFetchOutlet({ payload }: { type: string; payload: MenusOutletQuery }) {
  try {
...
export function* doEnableProductNotes({ payload }: { type: string; payload: ProductNotesQuery }) {
  try {
    const response = yield call(apiDoEnableProductNotes, payload);
    if (response.data && response.data.status) {
      yield put(
        actions.updateProductNotes({
          menuId: payload.menuId,
          outletId: payload.outletId,
...
export function* doEnableZNS({ payload }: { type: string; payload: ZNSQuery }) {
  try {
    const response = yield call(apiDoEnableZNS, payload);
    if (response.data && response.data.status) {
      yield put(
        actions.updateZNSStatus({
          outletId: payload.outletId,
