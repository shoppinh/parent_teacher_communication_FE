...
import { apiForgotPassword, apiForgotSetPassword, apiForgotVerifyCode, apiGetUserInfo, apiRegister, apiResendCode, apiSetPassword, apiVerifyCode } from 'services/api/apiHelper';
...
export function* doRegister({ payload }: any) {
  try {
    ...
    if(response.data && response.data.status) {
      yield put(
        actions.updateUser(ParseRegister(response.data.data))
      );
    } else {
...
export function* doVerifyCode({ payload }: any) {
    try {
      ...
      if(response.data && response.data.status) {
        yield put(
           actions.updateVerify(ParseVerify(response.data.data))
        );
      } else {
        yield put(actions.Error(response.data.error));
      }
  ...
  export function* checkAccountExists({ payload }: any) {
    try {
      ...
      if(response.data && response.data.status) {
        yield put(actions.checkedIsAccountExists({
