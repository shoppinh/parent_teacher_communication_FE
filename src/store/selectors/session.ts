...
export const getPhone = createSelector(
    [selectDomain],
    state => state.data.auth?.phoneNumber,
);

export const getUser = createSelector(
    [selectDomain],
    state => state.data.auth?.user?.data,
);
...
export const getFcmToken = createSelector(
    [selectDomain],
    state => state.data.auth?.fcmToken,
);
...
export const getSessionCountUnread = (menuGUID: string) => {
    return createSelector(
        [selectDomain],
        state => {
            if (state.data.auth?.user?.notifications) {
                return state.data.auth?.user?.notifications[menuGUID]
            }
            return undefined;
        }
    )
