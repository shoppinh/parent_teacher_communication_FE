...
export function isElementInView(el: (Element | null), isOutofView = false, alt = "") {
    ...
    if (el) {
        ...
        return (
            !isOutofView ? (rect.top >= 0 && rect.left >= 0 && rect.top <= (window.innerHeight || document.documentElement.clientHeight)) :
                ((rect.height - rect.top) >= 0 && (rect.width - rect.left) >= 0 && rect.top <= (window.innerHeight || document.documentElement.clientHeight))
        );
    }
    return false;
}
...
export const isMatchPerformanceToolUserAgent = () => {
    ...
    const rgx = new RegExp(/lighthouse|speedcurve|page speed|headless|webpagetest|screaming frog|addthis|PTST|googlebot|adsbot|feedfetcher|mediapartners|spider|bingbot|apica/g);
